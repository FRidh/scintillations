...
def _impulse_response_fluctuations(covariance, ntaps, window=None):
    # Covariance should be single side only.
...
def logamp_fluctuations(fluctuations, wavenumber, distance, correlation_length, mean_mu_squared, include_saturation=True):
...
def generate_fluctuations_logamp_and_phase(nsamples, fs, ntaps, correlation_length, speed, frequency, soundspeed,
    ...
    logamp = logamp_fluctuations(fluctuations[:,None], wavenumber, distance[:,None], correlation_length[:,None], mean_mu_squared[:,None], include_saturation=include_saturation)
    phase = phase_fluctuations(fluctuations[:,None], wavenumber, distance[:,None], correlation_length[:,None], mean_mu_squared[:,None])
...
def generate_fluctuations_spectra_and_delay(nsamples, fs, ntaps, correlation_length, speed, frequency, soundspeed,
    ...
    phase = phase_fluctuations(fluctuations[:,None], 2.*np.pi*1.0/soundspeed, distance[:,None], correlation_length[:,None], mean_mu_squared[:,None])
